...
**The review rides with the write decision — no extra prompt.** `collectCommonNames` runs the review when enabled and stashes the proposal in `bySource.llmProposal` (the working list stays deterministic — nothing applied, nothing logged). It returns `finalizeReview(accepted)`, and `app.js` ties the decision to the standard write/update confirmation (the same prompts as before the reviewer existed): an accepted write ("Apply available updates? [y/N]", or silent `--apply`/new-note creation) applies the diff to the merged names (Wikipedia-sourced entries only — names corroborated by Wikidata/GBIF survive) and records it in the tally log; a declined write or an already-complete note applies nothing and records nothing. New notes auto-create with the reviewed aliases when a proposal is pending. The proposal is visible beforehand in the Entity section's nested `(LLM review — pending): + ...; - ...` line. `--apply` writes and records as usual.
