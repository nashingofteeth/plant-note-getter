...
**One merged human gate at write time.** `collectCommonNames` runs the review when enabled and stashes the proposal in `bySource.llmProposal` (the working list stays deterministic — nothing applied, nothing logged). It returns `finalizeReview(accepted)`, and `app.js` folds the review decision into the note-write confirmation: for an existing note the Status section shows missing properties plus the proposal diff and asks once (`Apply updates + LLM review (+x/-y)? [y/N]`; review-only variant when there is nothing to update); for a new note the prompt appears before creation only when a proposal is pending (`Apply LLM review to the new note? [y/N]`) — clean runs auto-create as before. Accept → the diff is applied to the merged names (Wikipedia-sourced entries only — names corroborated by Wikidata/GBIF survive), the review is logged, and the note is written with the recomputed aliases. Decline → nothing is written, the deterministic list stands, nothing is recorded. `--apply` auto-accepts both gates.
