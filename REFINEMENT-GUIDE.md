...
- `qwen3:4b-instruct-2507-q4_K_M` — **recommended; best overall**. 8/8 Albizia junk (incl. cultivar entries), zero genuine-name losses, clean adds, silent controls. One defect (`diazotrophs` kept) fixed by the `other-organism` category.
...
**Post-refinement note (Sept 2026).** After the deterministic pipeline absorbed the reviewer's known territory (§9's tally loop, R63–R72), the remove pass became net-negative on heavy-prose articles: on Cichorium intybus it invents 13 variety-scope vetoes (with circular quotes), on Albizia 5–8 etymology/ambiguity vetoes — stable at temperature 0, insensitive to keep-rules and to a quote-justification requirement (the model generates circular quotes). Prompt tuning is exhausted; this is a 4B judgment ceiling. The reviewer stays useful for future gaps the regex doesn't yet cover, but expect false vetoes on prose-heavy naming sections — the human gate and the tally's false-veto list are the mitigation.

**Add-pass overflow on long articles (fixed Sept 2026).** On ~15k-char articles (Quercus robur) the add pass emitted a ~2,000-token avalanche of individual trees, galls, and repeated phrases that exceeded `num_predict: 2048`, truncated mid-string, and was silently discarded by `parseReviewJson` — a no-op after minutes of inference, on both model tags (prompt/budget issue, not model-specific). Fix: `ADD_SYSTEM_PROMPT` caps output at 10 names (`buildAddPrompt` repeats the cap), requires naming-construction evidence (naming verbs, appositives, common-name lists), and excludes individual organisms (`Major Oak`) and galls/diseases; a truncated completion now yields reason `llm-truncated` (plus a stderr warn) instead of silent `llm-empty`. Live check: 7,616-char truncated avalanche → 227-char parseable output in ~20s, individuals/repetition gone, short-article controls unaffected. Follow-up: 4 gall names still surfaced — root cause was placement, not wording (a "knopper gall is very common" passage overrode the mid-prompt exclusion). Fix: gall/individual exclusions lead the system prompt (primacy) and the `buildAddPrompt` task line repeats them. Live check on Quercus: exactly the true add set (`chêne pédonculé` — cultivars, product names like `French oak`, the `Royal Oak` individual, myth and place-etymology strings all excluded); recall probes (quoted miss, cultivar trap, Albizia control) all green. A stricter wording attempt mid-way collapsed output and was reverted — prefer placement over intensity.
...
**Review merged before the write confirmation; the write answer only gates logging.** `collectCommonNames` runs the review when enabled and merges the reviewed list into the working names right away — `names`, the Entity display, and the `Available updates` diff all include it (`bySource.llmAdded`/`llmRemoved` show the diff; names corroborated by Wikidata/GBIF survive removals because they were merged earlier). It returns a `logReview` closure (null when the model proposed nothing) — the tally record is appended only when the caller invokes it. `app.js` ties that to the write decisions: the update confirmation (`Apply available updates? [y/N]`) and the general creation confirmation (`Create note? [y/N]`, shown before any new note is created; `--apply` overrides both) — accepted write → review logged; denied write or note-not-created → nothing written and nothing recorded. The `(LLM review): + ...; - ...` line under the Wikipedia list shows what was merged.
