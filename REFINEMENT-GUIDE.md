...
- **Scope boundary: never change the model during refinement.** Switching `LLM_MODEL`, retuning backend options (`src/llm-backend.js`), or otherwise swapping/upgrading the completer is a project-level decision, not a per-taxon fix — it invalidates every prior probe and red test calibrated against the current model's behavior. Reviewer refinement means prompt wording only; model changes are out of scope for refinement runs.
