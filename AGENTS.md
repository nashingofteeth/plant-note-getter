...
| File | Role |
|------|------|
| `app.js` | CLI entry, orchestrates pipeline, supports `--check` mode; write/update prompts are the standard pre-LLM ones — a pending LLM review rides along (applied + tally-logged on write/create, dropped otherwise) |
...
| `src/names.js` | Common-name orchestration: `collectCommonNames` merges all sources, runs the end-of-Wikipedia LLM review when enabled (proposal stashed in `bySource.llmProposal`; returns `finalizeReview(accepted)` that applies/logs it — app.js merges that decision into the note-write confirmation), `buildAliases` produces final list |
...
| `src/llm-reviewer.js` | End-of-Wikipedia LLM review, two focused passes: (1) remove pass returns a **keep/remove verdict for every entry** in the deterministic list (key-match enforced, categories informational — keep-bias prompt, keep rules stated before remove rules), (2) add pass finds missed names against the **original** base list so it can't re-propose removals. Receives the extract, the taxon's scientific name + rank, and the base list. Decisions applied verbatim — trim, empty-filter, case-insensitive dedup; **no junk classifiers or other deterministic gates after the LLM**. Only Wikipedia-derived names are in scope. Pure, DI of the completer; missing/broken completer degrades to the deterministic list. Default model `qwen3:4b-instruct-2507-q4_K_M` (small + fast; keep-bias tuned for it; bake-off vs `qwen3.5:4b`/`qwen3.5:9b`/`gemma4:12b`/`glm-4.7-flash` in REFINEMENT-GUIDE §9 confirms it). Exports `reviewWikipediaNames`, `parseReviewJson`, `parseNamesJson`, `buildAddPrompt`/`buildRemovePrompt`, `ADD_SYSTEM_PROMPT`/`REMOVE_SYSTEM_PROMPT`, `REVIEWER_JSON_SCHEMA`, `REMOVE_JSON_SCHEMA`. |
