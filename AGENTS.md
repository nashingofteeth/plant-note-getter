...
| `src/names.js` | Common-name orchestration: `collectCommonNames` merges all sources, runs the end-of-Wikipedia LLM review (when `LLM_ENABLED` **and** a `reviewDecision` callback is supplied and accepts — no callback skips the LLM entirely; a declined proposal is applied nowhere and recorded nowhere), `buildAliases` produces final list |
...
| `src/llm-reviewer.js` | End-of-Wikipedia LLM review, two focused passes: (1) remove pass marks junk in the deterministic list (categories informational, key-match enforced), (2) add pass finds missed names against the **original** base list so it can't re-propose removals. Receives the extract, the taxon's scientific name + rank, and the base list. Decisions applied verbatim — trim, empty-filter, case-insensitive dedup; **no junk classifiers or other deterministic gates after the LLM**. Only Wikipedia-derived names are in scope. Pure, DI of the completer; missing/broken completer degrades to the deterministic list. Exports `reviewWikipediaNames`, `parseReviewJson`, `parseNamesJson`, `buildAddPrompt`/`buildRemovePrompt`, `ADD_SYSTEM_PROMPT`/`REMOVE_SYSTEM_PROMPT`, `REVIEWER_JSON_SCHEMA`. |
